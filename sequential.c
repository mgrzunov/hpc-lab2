    ...
    for (uint frame_i = 0; frame_i != FRAME_NUM; ++frame_i)
    {
        ...
        {            
            for (uint col = 0; col != PIXEL_WIDTH; col += 2)
            {
                ...
                yuv_undersampled_data[(row / 2) * (PIXEL_WIDTH / 2) + (col / 2) + (PIXEL_NUM + PIXEL_NUM / 4)] = (
                    yuv_frame_data[   row    * PIXEL_WIDTH + col     + (2 * PIXEL_NUM)] +
                    yuv_frame_data[   row    * PIXEL_WIDTH + col + 1 + (2 * PIXEL_NUM)] +
                    yuv_frame_data[(row + 1) * PIXEL_WIDTH + col     + (2 * PIXEL_NUM)] +
                    yuv_frame_data[(row + 1) * PIXEL_WIDTH + col + 1 + (2 * PIXEL_NUM)]
                ) / 4;
            }
        }
        byte_count = fwrite(
        ...
        for (uint row = 0; row != PIXEL_HEIGHT; row += 2)
        {
            for (uint col = 0; col != PIXEL_WIDTH; col += 2)
            {
                ...
                yuv_oversampled_data[   row    * PIXEL_WIDTH + col     + (2 * PIXEL_NUM)] =
                    yuv_undersampled_data[(row / 2) * (PIXEL_WIDTH / 2) + (col / 2) + (PIXEL_NUM + PIXEL_NUM / 4)]; 
                yuv_oversampled_data[   row    * PIXEL_WIDTH + col + 1 + (2 * PIXEL_NUM)] =
                    yuv_undersampled_data[(row / 2) * (PIXEL_WIDTH / 2) + (col / 2) + (PIXEL_NUM + PIXEL_NUM / 4)];
                yuv_oversampled_data[(row + 1) * PIXEL_WIDTH + col     + (2 * PIXEL_NUM)] =
                    yuv_undersampled_data[(row / 2) * (PIXEL_WIDTH / 2) + (col / 2) + (PIXEL_NUM + PIXEL_NUM / 4)];
                yuv_oversampled_data[(row + 1) * PIXEL_WIDTH + col + 1 + (2 * PIXEL_NUM)] =
                    yuv_undersampled_data[(row / 2) * (PIXEL_WIDTH / 2) + (col / 2) + (PIXEL_NUM + PIXEL_NUM / 4)];
            }
        }
        byte_count = fwrite(
